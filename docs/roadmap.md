...
- **Structured ingestion pipeline: substantially complete (2026-06-10)** — plan file `docs/plans/structured-ingestion.md`. **A** (frequency-based running-header/page-marker stripping in `cleaning.py`; root cause was an ordering bug that orphaned header text), **B.1–B.3** (docling PDF backend under the `[ingest]` extra; full-book evaluation vs the hand-curated TOC scored **98% heading recall in 34s** → **adopted**, `--pdf-backend auto` prefers docling when installed), and **C.1–C.3** (front/back-matter detection + `--trim-matter` on ingest — copyright/CONTENTS/bibliography/index, incl. headingless index-shape detection; docling headings drive structure detection so **`--toc` is now optional** for docling-ingested PDFs). End-to-end: full `Eco_1984_SPL.pdf`, no TOC file → 3,915 sentences, 134 structure nodes, headers gone, 820 matter lines auto-trimmed with itemized reporting. Remaining: C.4 QA fixture matrix; contents-page heading-anchor wart (noted in plan); nougat deferred.
- **Next up:** **sentence-transformers integration** (st.1–st.8 in Future Work) — generalize the `[embeddings]` plumbing + `cmapr similar TERM`; then the spaCy base-layer preprocessing migration. REBEL stays deferred — see next bullet.
