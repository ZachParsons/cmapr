...
- **Next up (reordered 2026-06-09): structured ingestion pipeline** — plan file `docs/plans/structured-ingestion.md` (Phase A: frequency-based running-header/page-marker stripping in `cleaning.py`; Phase B: docling backend evaluation vs the hand-curated TOC baseline; Phase C: front/back-matter classification + auto structure). Promoted ahead of sentence-transformers because the corpus tuning showed ingest noise (running headers fused mid-sentence) directly minting spurious edges — ingest quality now caps extraction quality, and cleaning lifts every downstream stage at once. **sentence-transformers** (st.1–st.8) follows; then the spaCy base-layer preprocessing migration. REBEL stays deferred — see next bullet.
